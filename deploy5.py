...
st.sidebar.markdown("---")
st.sidebar.markdown("### 🎯 Pricing Strategy Mode")
pricing_mode = st.sidebar.radio(
    "Select Pricing Mode:",
    [
        "⚡ Standard Best Price (Stock-Based Fulfillment)",
        "🎯 Very Best Price (Lowest Cost, Any Stock)"
    ],
    index=0,
    help="Choose whether to strictly fulfill from in-stock suppliers or find the absolute lowest supplier quotation."
)

zero_price_option = None
if pricing_mode == "🎯 Very Best Price (Lowest Cost, Any Stock)":
    zero_price_option = st.sidebar.radio(
        "Zero (0.00 AED) Price Rule:",
        [
            "🛡️ Exclude 0 AED (Lowest Non-Zero / 2nd Least)",
            "0️⃣ Consider 0 AED as Net Price"
        ],
        index=0,
        help="Choose whether to allow 0.00 AED quotes as the winning price or pick the lowest positive quote (2nd least)."
    )
...
st.sidebar.markdown(
    ...
    <div style="background: rgba(255, 255, 255, 0.05); padding: 14px; border-radius: 10px; border: 1px solid rgba(255, 255, 255, 0.1);">
        <p style="font-size: 0.78rem; color: #94a3b8; margin: 0; line-height: 1.5;">
            💡 <b>Pricing Rules:</b> Supports both inventory-backed fulfillment and unrestricted lowest-cost discovery with part supersession & live currency conversions.
        </p>
    </div>
    """,
    unsafe_allow_html=True
...
if not request_file or not matrix_file:
    ...
    for r_idx, row in df_matrix.iterrows():
        ...
        # Record all supplier options with valid numeric cost (cost >= 0)
        if not np.isnan(cost_val) and cost_val >= 0:
    ...
    for si_val in request_order:
        ...
        total_stock_available = sum(opt['stock'] for opt in item_data['options'] if opt['stock'] > 0)

        if pricing_mode == "🎯 Very Best Price (Lowest Cost, Any Stock)":
            # --- VERY BEST PRICE MODE (Unrestricted by Availability) ---
            if zero_price_option == "0️⃣ Consider 0 AED as Net Price":
                candidate_opts = [opt for opt in item_data['options'] if opt['cost'] >= 0]
            else:
                # Exclude 0 AED / Take 2nd least after 0 AED (Lowest positive quote)
                candidate_opts = [opt for opt in item_data['options'] if opt['cost'] > 0]
                if not candidate_opts:
                    # Fallback if no non-zero quote exists
                    candidate_opts = [opt for opt in item_data['options'] if opt['cost'] >= 0]

            # Sort by lowest cost first; if tied, sort by stock available descending
            candidate_opts = sorted(candidate_opts, key=lambda x: (x['cost'], -x['stock']))

            if candidate_opts:
                best_opt = candidate_opts[0]
                winning_row_indices.add(best_opt['original_df_row_idx'])
                chosen_unit_cost_aed = best_opt['cost']

                if chosen_unit_cost_aed > 0:
                    unit_cost_target  = chosen_unit_cost_aed * conversion_rate
                    selling_price     = unit_cost_target * (1.0 + (margin / 100.0))
                    final_unit_price  = round_up_to_two_decimals(selling_price)
                    final_total_price = round_up_to_two_decimals(final_unit_price * qty_needed)
                else:
                    final_unit_price  = 0.0
                    final_total_price = 0.0

        else:
            # --- STANDARD BEST PRICE MODE (Stock-Based Fulfillment) ---
            in_stock_options = [opt for opt in item_data['options'] if opt['stock'] > 0 and opt['cost'] > 0]
            sorted_options   = sorted(in_stock_options, key=lambda x: x['cost'])
            ...
            for opt in sorted_options:
                ...
                fulfilled_options.append({'cost': opt['cost'], 'take_qty': take_qty})
                selected_suppliers.append(f"{opt['supplier']} ({take_qty})")
                remaining_qty -= take_qty

            if total_fulfilled_qty > 0:
                # Unit price from the supplier that provided maximum quantity
                max_qty_opt          = max(fulfilled_options, key=lambda x: x['take_qty'])
                chosen_unit_cost_aed = max_qty_opt['cost']
